...
public class McashSeed
{
	...
	0x3779b99e, 0x6ef3733c, 0xdde6e678, 0xbbcdccf1, 0x779b99e3, 0xef3733c6, 0xde6e678d, 0xbcdccf1b
 	...
 	//private static boolean ENDIAN	 = BIG;	// IBM370, Motorola 68000, Pyramid (대부분의 RISC 기반)
	...
	// bugibugy add 
//	private static byte sUserKey[] = new String("!hanmir-mobilians!").getBytes();
	private static int  BuffSize    = 1024;  // 임시 버퍼 사이즈
	...
	{
//System.out.println("SeedEncRoundKey들어옴...");		
		int A[] = new int[1];
		int B[] = new int[1];
		int C[] = new int[1];
		int D[] = new int[1];
...
//System.out.println("암호화내부 e1");
		int	rt, end;
		String rtn = "";
		int sRoundKey[] = new int[32];
//System.out.println("암호화내부 e2");		
/*
		SeedEncRoundKey( sRoundKey, sUserKey );
*/
		//szKey = new String("!hanmir-mobilians!").getBytes();
		SeedEncRoundKey( sRoundKey, szKey );
//System.out.println("암호화내부 e3");				
// 입력 받은 String을 Byte 배열에 담는다.
		byte	sbuffer[] = inData.getBytes();
		rt = (sbuffer.length + SeedBlockSize - 1) / SeedBlockSize;

//  SeedBlockSize 크기 별로 암호화 하기
		...
		for( int j = 0; j < rt; j++)
		{
			...
			if ( pos1 - j != 0 ) {
				...
				for ( s = j * SeedBlockSize ; s < end ; s++, pos++)	{
...
//System.out.println("암호화내부 e4");		
	...
	public static String decodeString( String inData,  byte szKey[])	{
		...
		//szKey = new String("!hanmir-mobilians!").getBytes();		
		SeedEncRoundKey( sRoundKey, szKey );		
//		SeedEncRoundKey( sRoundKey, "0007300100073001".getBytes() );		

		// 입력 받은 String을 Byte 배열에 담는다.
		byte	sbuffer[] = inData.getBytes();
		rt = sbuffer.length  / ( SeedBlockSize * 2 );
...
//		System.out.println( "1111");
		...
		for(j = 0; j < rt; j++)
		{
			...
			k = j * SeedBlockSize * 2  + SeedBlockSize * 2;
			for (int s = j * SeedBlockSize * 2 ; s < k ; s++, l++)	{
				sTmpBuf[l] = sbuffer[s];
			}

			for ( int m = 0, n =0 ; m < ( 2 * SeedBlockSize ) ; m = m + 2, n++)	{
			...
			for ( int o= 0; o < SeedBlockSize; o ++ , p ++ )
				temp[p] = sTarget[o];
